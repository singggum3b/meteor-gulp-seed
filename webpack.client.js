...
module.exports = function (settings) {
	...
	return {
		...
		output: {
			...
			publicPath: isProd ? undefined : settings.devhost + settings.publicPath,
			filename: "[name].js",
		...
		module: {
			...
			loaders: [
				...
				{test: /\.(svg|png|gif|jpe?g|ico)(\?.*)?$/, loader: "url?limit=8182"},
				{test: /\.(ttf|woff|eot|otf)(\?.*)?$/, loader: "file"}
			],
			postLoaders: [
		...
		plugins: [
			new webpack.DefinePlugin({
				"process.env": {
					...
					"WEBPACK_PUBLICPATH" : JSON.stringify(isProd ? "" : settings.devhost + settings.publicPath),
